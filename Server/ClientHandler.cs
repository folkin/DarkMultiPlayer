...
namespace DarkMultiPlayerServer
{
    public class ClientHandler
    {
        ...
        private static void SetupTCPServer()
        {
            try
            {
                IPAddress bindAddress = IPAddress.Parse(Server.serverSettings.Settings.address);
                TCPServer = new TcpListener(new IPEndPoint(bindAddress, Server.serverSettings.Settings.port));
                try
                {
        ...
        internal static void DisconnectClient(ClientObject client)
        {
            lock (client.disconnectLock)
            {
                ...
                if (clients.Contains(client))
                {
                    ...
                    Server.players = GetActivePlayerNames();
                    DarkLog.Debug("Online players is now: " + Server.playerCount + ", connected: " + clients.Count);
                    if (!Server.serverSettings.Settings.keepTickingWhileOffline && clients.Count == 0)
                    {
